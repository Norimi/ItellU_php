...
$id_groups = $jsonArray['id_group'];
$applying = false;

$stmt = $pdo->prepare('SELECT id_friend, modified FROM Relation WHERE IN("'.implode('","', $id_groups).'") AND id_friend = 0 AND modified > "$newest_modified" ORDER BY modified DESC');
$stmt->execute();
$all = $stmt->fetchAll();
